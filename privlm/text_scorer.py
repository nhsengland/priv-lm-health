"""
Adapted from package mincons: https://github.com/kanishkamisra/minicons. Thank you!
"""
...
class LMScorer:
    ...
    def __init__(
        ...
        # {self.vocab[x.strip()].append(i) for x, i in [
        #     (self.tokenizer.decode([i]), i) for i in range(self.tokenizer.vocab_size)
        # ]}
    ...
    def token_score(
        ...
        where score represents the log-probability (by default) of the token given
        context. Can also return ranks along with scores.

        :param ``Union[str, List[str]]`` batch: a single sentence or a batch of
        sentences.
        :param ``bool`` surprisal: If `True`, returns per-word surprisals instead of
        log-probabilities.
        :param ``bool`` prob: If `True`, returns per-word probabilities instead of
        log-probabilities.
        :param ``bool`` base_two: If `True`, uses log base 2 instead of natural-log
        (returns bits of values in case of surprisals)
        :param ``bool`` rank: If `True`, also returns the rank of each word in context
        (based on the log-probability value)

        :return: A `List` containing a `Tuple` consisting of the word, its associated
        score, and optionally, its rank.
    ...
    def score(
        ...
        """
        DEPRECATED as of v 0.1.18. Check out ``sequence_score`` or ``token_score``
        instead!
    ...
    def conditional_score(
        ...
        **kwargs,
        ...
        given a prefix. Pooling is usually done using a function that is passed to the
        method.

        :param prefix: a batch of prefixes or primes passed to the
            language model. This is what the sequence is conditioned on, and the model
            ignores the word probabilities of this part of the input in estimating the
            overall score.
        ...
        :param reduction: Reduction function, is selected to be
            ``lambda x: x.mean(0).item()`` by default, which stands for the avg.
            log-probability per token for each sequence in the batch.
        :type reduction: Callable
        :param kwargs: parameters for the ``compute_stats`` call --

            * `prob` (`bool`): Whether the returned value should be a probability
            (note that the default reduction method will have to be changed to
            `lambda x: x.prod(0).item()` to get a meaningful return value)

            * `base_two` (`bool`): whether the returned value should be in base 2
            (only works when `prob = False`)

            * `surprisal` (`bool`): whether the returned value should be a surprisal
            (does not work when `prob = True`)


        :return: List of floats specifying the desired score for the stimuli part of
        the input, e.g., P(stimuli | preamble).
        :rtype: ``List[float]``
        """
...
class MaskedLMScorer(LMScorer):
    ...
    def prepare_text(
        ...
        :param PLL_metric: PLL scoring strategy to be used.
            ...
            For motivation as to why to use `within_word_l2r` PLL scoring,
            see Kauf & Ivanova (2023):
            https://arxiv.org/abs/2305.10588

        :return: Batch of formatted input that can be passed to `logprob`
        """
        # converts input text to batch of tensors with every position except the
        # cls and sep token masked
        ...
        for ind, (token_ids, attention_mask) in enumerate(
            ...
            if PLL_metric == "within_word_l2r":
                """
                Future tokens belonging to the same word as the target token are
                masked during token inference as well.
                """
                ...
                # sample 15% of indices from a list 10 times without replacement
                n = math.floor(0.15 * effective_length)
                if n < 1:
                    n = 1
                number_of_masked_sequences = math.floor(effective_length / n)
                possible_mask_indices = [
                    mask_pos for mask_pos in range(effective_length + 2)
                ][1:-1]
                mask_indices = []
                for i in range(number_of_masked_sequences):
                    samples = random.sample(possible_mask_indices, n)
                    possible_mask_indices = [
                        x for x in possible_mask_indices if x not in samples
                    ]
                    mask_indices.append(samples)
                # a = 1
            else:
                # Original PLL metric
    ...
    def prime_text(
        ...
        :param ``Union[str, List[str]]`` preamble: Batch of prefixes/prime/preambles
        on which the LM is conditioned.
        :param ``Union[str, List[str]]`` stimuli: Batch of continuations that are
        scored based on the conditioned text (provided in the ``preamble``). The
        positions of the elements match their counterparts in the ``preamble``.
    ...
    def logprobs(
        ...
        :rtype: Union[List[Tuple[torch.Tensor, str]],
    ...
    def compute_stats(
        ...
        """
        Primary computational method that processes a batch of prepared sentences and
        returns per-token scores for each sentence. By default, returns
        log-probabilities.

        :param ``Iterable`` batch: batched input as processed by ``prepare_text``
        or ``prime_text``.
        :param ``bool`` rank: whether the model should also return ranks per
        word (based on the conditional log-probability of the word in context).
        :param ``bool`` prob: whether the model should return probabilities instead
        of log-probabilities. Can only be `True` when `base_two` is `False`.
        :param ``bool`` base_two: whether the base of the log should be 2 (usually
        preferred when reporting results in bits). Can only be `True` when `prob`
        is `False`.
        :param ``bool`` return_tensors: whether the model should return scores as
        a list of tensors instead of a list of lists. This is important in some other
        convenient methods used in the package.

        :return: Either a tuple of lists, each containing probabilities and ranks per
        token in each sentence passed in the input.
        :rtype: ``Union[Tuple[List[float], List[float]], List[float]]``
        """
        assert not (base_two and prob), (
            "cannot both use base (which is for a log), and a probability measure at"
            " the same time!"
        )
    ...
    def sequence_score(
        ...
        """
        TODO: reduction should be a string, if it's a function, specify what kind of
        function. --> how to ensure it is always that type?
        """
    ...
    def token_score(
        ...
        :param ``str`` PLL_metric: PLL scoring strategy to be used.
            Options: `original` or `within_word_l2r`. Default: `original`
            For motivation as to why to use `within_word_l2r` PLL scoring, see
            Kauf & Ivanova (2023):
            https://arxiv.org/abs/2305.10588
...
class IncrementalLMScorer(LMScorer):
    """
    Class for Autoregressive or Incremental (or left-to-right) language models such as
    GPT2, etc.
    ...
    def compute_stats(
        ...
        :param ``Iterable`` batch: batched input as processed by ``prepare_text``
        or ``prime_text``.
        :param ``bool`` rank: whether the model should also return ranks per word
        (based on the conditional log-probability of the word in context).
        :param ``bool`` prob: whether the model should return probabilities instead of
        log-probabilities. Can only be `True` when `base_two` is `False`.
        :param ``bool`` base_two: whether the base of the log should be 2 (usually
        preferred when reporting results in bits). Can only be `True` when `prob`
        is `False`.
        :param ``bool`` return_tensors: whether the model should return scores as
        ...
        for logit, idx, offset in zip(logits, effective_ids, offsets):
            ...
            if rank:
                ...
                TODO: Try to implement ranking in linear time but across arbitrary
                dimensions:
                https://stackoverflow.com/a/5284703
                """
                word_ranks = (-1.0 * logprob_distribution).argsort().argsort() + 1
        ...
        if return_tensors:
            scores = [torch.tensor(lr) for lr in scores]
    ...
    def logprobs(self, batch: Iterable, rank=False) -> Union[float, List[float]]:
        ...
        for sent_index in range(len(ids)):
            ...
            # select only the ids present in the sentence out of all vocab items
            # (as a 2d array)
    ...
    def fixed_label_score(
        ...
        """
        labels_with_space = [f" {lr.strip()}" for lr in labels]
...
class Seq2SeqScorer(LMScorer):
    """
    Class for Autoregressive or Incremental (or left-to-right) language models such
    as GPT2, etc.
    ...
    def compute_stats(
        ...
        preferred when reporting results in bits). Can only be `True` when `prob`
        is `False`.
        :param ``bool`` return_tensors: whether the model should return scores as a
        list of tensors instead of a list of lists. This is important in some other
        convenient methods used in the package.
    ...
    def sequence_score(
        ...
        """
        TODO: reduction should be a string, if it's a function, specify what kind
        of function. --> how to ensure it is always that type?
        """
